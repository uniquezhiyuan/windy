...
def forecast(name, position, driver):
    ...
    DAY_FIRST = int(day.text.split(' ')[1])
    ...
    start_date_hour = datetime(YEAR, MONTH, DAY_FIRST, HOUR_FIRST)
    day_hour_list = []
    for i in range(48):
    ...
    tempature_list = []
    for i in range(48):
        tempature_list.append(tempatures[i][:-1])
    ...
    for i in rains:
        if 'cm' in i:
            rain_list.append(str(float(i[:-2]) * 10))
        else:
            if i == '':
                rain_list.append('0')
            else:
